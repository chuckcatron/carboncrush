...
export async function POST(request: NextRequest) {
  try {
    ...
    // Search for green businesses using coordinates
    ...
    console.error('Error searching places by coordinates:', error);
    return NextResponse.json(
...
async function searchGreenBusinesses(lat: number, lng: number, category: string, query: string, apiKey: string) {
  ...
  for (const searchTerm of searchTerms) {
    try {
      // Use Places API Nearby Search for better location-based results
      const searchUrl = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${lat},${lng}&radius=10000&keyword=${encodeURIComponent(searchTerm)}&key=${apiKey}`;
      ...
      if (data.status === 'OK' && data.results) {
        for (const place of data.results.slice(0, 5)) { // Limit results per search
          ...
          if (details) {
            const businessWithSustainability = await enhanceWithSustainabilityData(details, category, lat, lng);
...
async function enhanceWithSustainabilityData(place: any, category: string, userLat: number, userLng: number) {
  ...
  return {
    ...
    distance: calculateDistance(place.geometry?.location, userLat, userLng)
  };
}
...
function calculateDistance(businessLocation: any, userLat: number, userLng: number) {
  ...
  const dLat = toRad(businessLocation.lat - userLat);
  const dLng = toRad(businessLocation.lng - userLng);
  
  const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) +
    Math.cos(toRad(userLat)) * Math.cos(toRad(businessLocation.lat)) *

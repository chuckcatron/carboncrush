...
export default function AIRecommendations({ carbonData, results, onRecommendationUpdate }: AIRecommendationsProps) {
    ...
    const fetchProviders = async () => {
      try {
        const response = await fetch('/api/recommendations', {
          ...
          body: JSON.stringify({ checkProviders: true })
        });
  ...
  const generateRecommendations = useCallback(async () => {
    ...
    try {
      ...
      if (data.provider === 'fallback') {
        ...
        const providerName = data.provider === 'claude' ? 'Claude' : 'GPT-4o';
        const timeText = data.processingTime ? ` in ${data.processingTime}ms` : '';
        toast.success(`AI recommendations generated by ${providerName}${timeText}!`);
      }
  ...
  return (
    ...
    >
          ...
          {availableProviders.length > 1 && (
            <div className="relative">
              ...
              >
                ...
                <span>AI</span>
                <ChevronDown className={`w-4 h-4 transition-transform ${showProviderSelector ? 'rotate-180' : ''}`} />
              </button>
              
              {showProviderSelector && (
                <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-lg shadow-lg border border-slate-200 z-10">
                  <div className="p-3 border-b border-slate-200">
                    <h4 className="font-medium text-slate-900">Choose AI Provider</h4>
                  </div>
                  {availableProviders.map((provider) => (
                    <button
                      ...
                      className={`w-full p-3 text-left hover:bg-slate-50 flex items-center space-x-3 ${
                        selectedProvider === provider.name ? 'bg-emerald-50 border-r-2 border-emerald-500' : ''
                      }`}
                    >
                      <span className="text-lg">{provider.name === 'claude' ? '🧠' : '⚡'}</span>
                      <div>
      ...
      {currentProvider && recommendations.length > 0 && (
        <div className="mb-6 p-3 bg-slate-50 rounded-xl border border-slate-200">
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center space-x-2">
              <span className="text-lg">
                {currentProvider === 'claude' ? '🧠' : currentProvider === 'gpt4o' ? '⚡' : '🔧'}
              </span>
              <span className="text-slate-600">
                Generated by {' '}
                <span className="font-medium text-slate-900">
                  {currentProvider === 'claude' ? 'Claude (Anthropic)' : 
                   currentProvider === 'gpt4o' ? 'GPT-4o (OpenAI)' : 
                   'Fallback System'}
                </span>
              </span>
            </div>
